...
PERIODS = {
    "period1": {
        ...
        "end": date(2026, 1, 29),
        "expected": {age: (p(age + 1), p(age + 2)) for age in range(18, 41)},
    },
    "period2": {
        "start": date(2026, 1, 31),
        "end": date(2026, 3, 19),
        "expected": {age: (p(age + 1), p(age - 9)) for age in range(18, 41)},
    },
}
...
def main():
    ...
    with RAW_CSV.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if (row.get("os") or "").strip().lower() != "android":
                continue
            if (row.get("region") or "").strip().lower() != "rich":
                continue
    ...
    payload = {"mode": "ab", "overall": {}, "periods": {"period1": {}, "period2": {}}}
    period_days = {
    ...
    with (DASHBOARD_DIR / "dashboard_data_rich_region_android.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(dashboard_rows[0].keys()))
        writer.writeheader()
        writer.writerows(dashboard_rows)
    with (DASHBOARD_DIR / "daily_cvr_android_rich_mar19.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    summary_json["android_rich"] = payload
