...
EXCLUDED_DATES = {date(2026, 1, 12), date(2026, 1, 30)}
ISO3_MANUAL = {
    "XKS": "Kosovo",
    "GBR": "United Kingdom",
    "USA": "United States",
...
PERIODS = {
    "period1": {
        ...
        "end": date(2026, 1, 11),
        "expected": {age: (f"P{age - 9}", f"P{age - 8}") for age in range(18, 41)},
    },
    "period2": {
        "start": date(2026, 1, 13),
        "end": date(2026, 1, 29),
        "expected": {
            18: ("P9", "P5"), 19: ("P10", "P5"), 20: ("P11", "P5"), 21: ("P12", "P5"),
            22: ("P13", "P10"), 23: ("P14", "P10"), 24: ("P15", "P10"),
            25: ("P16", "P14"), 26: ("P17", "P14"), 27: ("P18", "P14"), 28: ("P19", "P14"), 29: ("P20", "P14"),
            30: ("P21", "P20"), 31: ("P22", "P20"), 32: ("P23", "P20"), 33: ("P24", "P20"), 34: ("P25", "P20"),
            35: ("P26", "P28"), 36: ("P27", "P28"), 37: ("P28", "P28"), 38: ("P29", "P28"), 39: ("P30", "P28"), 40: ("P31", "P28"),
        },
    },
    "period3": {
        ...
        "end": date(2026, 3, 19),
        "expected": {
            18: ("P5", "P5"), 19: ("P5", "P5"), 20: ("P5", "P5"), 21: ("P5", "P5"),
            22: ("P10", "P10"), 23: ("P10", "P10"), 24: ("P10", "P10"),
            25: ("P14", "P12"), 26: ("P14", "P12"), 27: ("P14", "P12"), 28: ("P14", "P12"), 29: ("P14", "P12"),
            30: ("P20", "P12"), 31: ("P20", "P12"), 32: ("P20", "P12"), 33: ("P20", "P12"), 34: ("P20", "P12"),
            35: ("P28", "P20"), 36: ("P28", "P20"), 37: ("P28", "P20"), 38: ("P28", "P20"), 39: ("P28", "P20"), 40: ("P28", "P20"),
        },
    },
}
...
def main():
    ...
    with RAW_CSV.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if (row.get("os") or "").strip().lower() != "android":
                continue
            if (row.get("region") or "").strip().lower() != "poor":
                continue
            ...
            apval = f"P{(row.get('apval') or '').strip()}" if (row.get("apval") or "").strip().isdigit() else (row.get("apval") or "").strip()
            bpval = f"P{(row.get('bpval') or '').strip()}" if (row.get("bpval") or "").strip().isdigit() else (row.get("bpval") or "").strip()
            if (apval, bpval) != expected:
                continue
    ...
    for duration in DURATIONS:
        payload["overall"][duration] = {
            group: finalize_bucket(summary[("overall", duration, group)], total_days)
            for group in ("A", "B")
        }
    for period_name, cfg in PERIODS.items():
        ...
        for duration in DURATIONS:
            payload["periods"][period_name][duration] = {
                group: finalize_bucket(summary[(period_name, duration, group)], days)
                for group in ("A", "B")
            }

    with (DASHBOARD_DIR / "dashboard_data_poor_region_android.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    with (DASHBOARD_DIR / "daily_cvr_android_poor_mar19.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
    ...
    full_summary = json.loads(SUMMARY_JSON.read_text())
    full_summary["android_poor"] = payload
    SUMMARY_JSON.write_text(json.dumps(full_summary, indent=2))

    print(json.dumps(payload["overall"]["All"], indent=2))
    print(json.dumps(payload["periods"], indent=2)[:5000])
