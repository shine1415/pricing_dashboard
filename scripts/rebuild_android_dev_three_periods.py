...
PERIODS = {
    "period1": {
        ...
        "end": date(2026, 1, 12),
        "expected": {age: (p(age - 5), p(age - 4)) for age in range(18, 41)},
    },
    "period2": {
        "start": date(2026, 1, 14),
        "end": date(2026, 1, 29),
        "expected": {age: (p(age - 5), p(age - 9 if age < 40 else 30)) for age in range(18, 41)},
    },
    "period3": {
...
def main():
    ...
    with RAW_CSV.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if (row.get("os") or "").strip().lower() != "android":
                continue
            if (row.get("region") or "").strip().lower() != "dev":
                continue
    ...
    payload = {"mode": "production", "overall": {}, "periods": {"period1": {}, "period2": {}, "period3": {}}}
    period_days = {
    ...
    with (DASHBOARD_DIR / "dashboard_data_dev_region_android.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(dashboard_rows[0].keys()))
        writer.writeheader()
        writer.writerows(dashboard_rows)
    with (DASHBOARD_DIR / "daily_cvr_android_dev_mar19.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    summary_json["dev_android"] = payload
